...
<template>
    <AuthenticatedLayout>
        <h1 class="text-4xl text-center font-semibold">{{ budget.data.name }}</h1>
        <div class="mt-8 text-center">
            <h3 class="text-2xl font-semibold">${{ (budget.data.amount / 100).toFixed(2) }}</h3>
            <h3 class="text-xl mt-2" v-if="budget.data.show_daily_amount">(~${{ (budget.data.average_daily_amount / 100).toFixed(2) }}/day)</h3>
