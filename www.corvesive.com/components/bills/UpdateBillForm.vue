...
<template>
  <div class="w-11/12 max-w-lg mx-auto">
    <h3 class="text-xl font-bold mb-4">{{ form.issuer }} - {{ form.name }}</h3>
    <form @submit.prevent="useBillsStore().updateBill" class="flex flex-col gap-4">
      <FormsDoubleInput>
        <div>
      ...
      <div class="flex gap-4">
        <ButtonsFormDeleteButton
          buttonText="Delete"
          :disabled="form.isLoading"
          @click="useBillsStore().deleteBill"
        />
